...
def most_goals(players: list):
    return max(sorted(players, key=lambda player: player.goals)).name

def most_points(players: list):
    best = max(sorted(players, key=lambda player: player.goals + player.passes))
    return (best.name, best.number)

def least_minutes(players: list):
    return min(sorted(players, key=lambda player: player.minutes))
