...
def sort_by_number_of_routes(areas: list):
    def route_sort(area: ClimbingArea):
        return area.routes()
    return sorted(areas, key=route_sort)

def sort_by_most_difficult(areas: list):
    def difficulty_sort(area: ClimbingArea):
        return area.hardest_route().grade
    return sorted(areas, key=difficulty_sort, reverse=True)